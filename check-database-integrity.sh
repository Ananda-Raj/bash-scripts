...
#Change the path to the required; where the db dumps are stored
path_db=/home/mysqlbackup
...
find $path_db -name "*.sql.gz" -type f -mtime -1 > /usr/local/mysql-temp/$(date +%d)-database-list.log
